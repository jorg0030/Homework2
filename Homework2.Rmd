...
# Scenario 1
a) Na: The mean catch (in tonnes) has changed between 2009 and 2010
No: the mean shrimp catch was the same in 2009 and 2010
...
b)
```{r}
hist(FishCatch$X2009.catch.tonnes,main = "Shrimp Catch 2009", xlab = "Annual Shrimp Catch (Tonnes)", ylab = "Number of communities surveyed", col="gold")

hist(FishCatch$X2010.catch.tonnes,main = "Shrimp Catch 2010", xlab = "Annual Shrimp Catch (Tonnes)", ylab = "Number of communities surveyed", col="orange", ylim = c(0,4))
```

c) A paired two sample to test should be used to test the hypothesis as there are two mean values tro compare. The test is paired because the data was collecged at the same lcoations.
...
qt(0.95,13)
```
1.7709>-6.3808 so tcrit > tobserved
...
a) Ho: The phosphorus levels in the lake are signifcantly different than teh guideline
h1: the phosphorus levels in the lake are not signifcantly different than the guidline
...
c) A one-sample t-test should be used to test the hypothesis as it involved comparing the data mean to a specific given value.A two tailed t-test will be used as the phosphorus levels can either be above or below the reocommended. 
...
```
c) A paired two sample t-test should be used as we are comparing two populations with different means, but they are within the same enviornment otherwise (same farm)

d) 
```{r}
t.test(Veg, Carnivor, paired=T)
```
...
1.761 > -15.162 so tcrit > tobserved, and p << 0.05
...
qchisq(0.95,2)
```
chisq crit << chisq crit and p << 0.05

f) There is a significant difference of the impacts of PCB depending on trophic level
